...
const ExportDataModal: React.FC<ExportDataModalProps> = ({ users, onClose, strings }) => {
    ...
    const handleExport = async () => {
        ...
        if (exportFormat === 'csv') { // This is now the Image Export
            ...
            try {
                root.render(<ExportReport dataByUser={dataForReport} filters={filters} strings={strings} />);
                await new Promise(resolve => setTimeout(resolve, 500));
                const reportElement = tempContainer.querySelector<HTMLElement>('#export-report-content');
            ...
            const dataByUser: UserReportData[] = Array.from(filteredDataByUser.entries()).map(([userName, data]) => ({
                userName, data,
            })).sort((a, b) => a.userName.localeCompare(b.userName));

            const selectedUsers = users.filter(u => activeUserIds.includes(u.id));
            const userFilterNames = (selectedUsers.length === users.length && users.length > 1)
                ? ['All Users']
            ...
            try {
                ...
                for (let i = 0; i < dataByUser.length; i++) {
                    ...
                    root.render(<ExportReport dataByUser={[userReport]} filters={filters} isPdfPage={true} strings={strings} />);
